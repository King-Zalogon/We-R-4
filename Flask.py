...
class Profesor:
    ...
    def __init__(self,nombre,apellido,email,nacimiento,provincia,descripcion,materia,foto):
        self.id =  None
        self.nombre = nombre
    ...
    def modificar(self,nueva_provincia,nueva_descripcion,nueva_materia,nueva_foto):
...
class Profesores:
    ...
    def agregar(self,nombre,apellido,email,nacimiento,provincia,descripcion,materia,foto):
        conect = conectar()
        cursor = conect.cursor()
        ...
        nuevo_profesor = Profesor(nombre,apellido,email,nacimiento,provincia,descripcion,materia,foto)
        cursor.execute("""INSERT INTO profesores VALUES (?,?,?,?,?,?,?,?)""",
                           (nombre,apellido,email,nacimiento,provincia,descripcion,materia,foto))
        conect.commit()
        
        return jsonify({"mensaje":"El registro se ha completado con éxito"}),200
    
    def modificar_profe(self,email,nueva_provincia,nueva_descripcion,nueva_materia,nueva_foto):
        conect = conectar()
        cursor = conect.cursor()
        profesor_existente = self.consultar_profe(email)
        
        if profesor_existente:
            profesor_existente.modificar(nueva_provincia,nueva_descripcion,nueva_materia,nueva_foto)
            cursor.execute("UPDATE profesores SET provincia = ?, descripcion = ?, materia = ?, foto = ? WHERE email = ?",(nueva_provincia,nueva_descripcion,nueva_materia,nueva_foto,email))
            conect.commit()
    ...
    def listar_profesores(self):
        ...
        for fila in filas:
            nombre,apellido,email,nacimiento,provincia,descripcion,materia,foto = fila
            profesor = {"nombre":nombre,"apellido":apellido,"email":email,"nacimiento":nacimiento,"provincia":provincia,"descripcion":descripcion,"materia":materia,"foto":foto}
...
@app.route("/profesor/< str:email >",methods = ["PUT"])
def editar_perfil(email):
    ...
    return profesores.modificar_profe(email,nueva_provincia,nueva_descripcion,nueva_materia,nueva_foto)

@app.route("/profesor/< str:email >", methods = ["DELETE"])
def eliminar(email):
